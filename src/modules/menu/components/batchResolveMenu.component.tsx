...
export function BatchResolveMenu({batchStats, batchSelect, batchMethod, batchAction}:{
    ...
    return <React.Fragment>
        ...
        <Section title='Method' disabled={batchStats.selectedCount===0}>
            ...
            <Button title={'Unset method'} tooltip={'Unset resolution method for selected dedupes'} onClick={()=>batchMethod(ResolutionMethodType.sum)} data-testid='batch_method_unset' disabled={batchStats.selectedCount===0||(batchStats.readyToResolve===0&&batchStats.alreadyResolved===0)}/>
        </Section>

        <Section title='Action' disabled={batchStats.selectedCount===0}>
            <Button title={'Execute'} tooltip={'Save and upload the chosen resolution for all selected dedupes'} onClick={()=>batchAction(BatchActionType.resolve)} data-testid='batch_action_resolve' disabled={batchStats.readyToResolve===0} color='secondary' /*variant='outlined'*/ variant={'contained'} disableElevation/>
        </Section>
    </React.Fragment>
}